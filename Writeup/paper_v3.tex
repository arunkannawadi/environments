...
photometric redshifts from their observed\claire{strike} colors. 
Given the expected sub-per cent \claire{statistical} errors on upcoming surveys, systematic errors must be reduced from
...
representing\claire{strike} the current widest contiguous area surveyed by the {\em HST}, and the
...
atypical depending on the environment in that slice.  This would have\claire{has}
...
simulations, following the approach taken to this problem\claire{strike: to this problem} in
...
of imaging data from which\claire{used} to determine high-fidelity photometric
redshifts. 
...
in each bin is much lesser compared\claire{less than} the difference between the actual number of galaxies present
...
in detail below) is\claire{are} considered. This threshold is set on the
...
complete to $m_\text{F814W}=25.2$,(\claire{I'm not sure how the parent sample comes into play here}) we compare the $M_I$ distribution
...
bracket reality, \claire{strike the rest}which should be somewhere in between.
...
redshift evolution, \s$2$ includes  more galaxies. (\claire{How many?})
...
function(\claire{the LF as a function of redshift, right?}) for our sample, thus simplifying this exercise.  However,
...
As for $M_I$, we compare the stellar mass function in\claire{of} the $m_\text{F814W}\le23.5$ sample with that in\claire{of} the $m_\text{F814W}\le25.2$ sample. 
...
discussed in this section for\claire{of} obtaining a volume-limited sample. The
...
\begin{figure*}
 ...
 \caption{Stellar mass distribution for various flux-limited samples
   ...
   bins have been chosen to facilitate the comparison with  
          ...
          (\claire{middle: Why does your mass function go above Tomczak at higher redshift? If you aren't comparing to it directly, maybe just leave it out and do an internal comparison. Would including errors bars make the agreement better?})
          }
 \label{fig:smf}
\end{figure*}
...
this\claire{strike:when we do this}, the values of $\delta_{g,\text{1D}}$ for the $z=0.40-0.55$ bin
...
Fig.~\ref{fig:redshift_vollimited} as in\claire{and} Fig.~\ref{fig:redshift_fluxlimited}. 
...
\begin{figure}
 ...
 \caption{RMS distortions as a function of redshift, with points
   ...
   case, the distortions are the\claire{from} moments-based shape estimates, rather
   than from fits to \sersic profiles.\claire{fits.} }
...
\claire{Maybe don't use red and green. It's not an uncommon color-blindness.} For figures up to Fig.~\ref{fig:median_dvcbtt}, the colors of the points were selected to easily differentiate between galaxies in overdense, neutral, and underdense redshift slices.
...
underdense, but has somewhat\claire{strike} low RMS ellipticities\claire{ellipticity} that agree\claire{s} with the rest of the overdense regions.

Next, we show an analogous plot of RMS ellipticities\claire{\claire{You mean distort ions? Check that you are consistent throughout this section.}} %\arun{Is this ellipticity or distortion?} 
for all three
...
However, it is generally the case that\claire{strike:it is generally the case that} the statistical significance of
...
tricky\claire{highlight} because both distributions have hard cutoffs that are enforced
...
environmentally neutral when we volume-limit the sample to be complete upto\claire{highlight} $z=1$.
...
median \sersic index for there\claire{} bins in
...
dominant in its size, the combination with\claire{of} other much smaller fields
...
RMS ellipticity at the level of 0.01 (\claire{That's the level you find, correct?}) would become fractional shear
errors of
\begin{equation}
