...
$B/T\ge $ or $<0.4$.  The resulting bulge fraction $f_\text{bulge}$,
...
\begin{figure}
 ...
 \caption{\rachelnew{The fraction of galaxies in \s1 (open symbols) and
     \s3 (solid symbols) that are classified as bulge-dominated as a
...
environment dependence seen here.  We showed in
...
distribution, but testing whether it (or a more complex type split) is
...
complicated analysis that is beyond the scope of this work.} It seems that a future study using
  ...
  test whether the \rachelnew{ possible} discrepancy between our findings and those of
  ...
  problem with colour gradient calibration), \rachelnew{or whether a
    division into several galaxy types is truly enough to remove the colour
    gradient effect, verifying the conclusions of
...
\rachelnew{factor of 9 (for AEGIS, and more for GOODS).  The GEMS field
  comes the closest to that of COSMOS, but is still a factor of 6
  smaller and may be less useful for determination of colour gradients
  given that one of the bands, F850LP, is relatively shallow.}.
