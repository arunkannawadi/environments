...
photometric redshifts from their observed\claire{strike} colors. (\arun{I don't see any problems with `observed'. Maybe just say observed galaxy shapes?})
...
(\claire{I'm not sure how the parent sample comes into play here} 
 \arun{The catalog is upto 25.2. We restrict to 23.5 since fainter ones have bad Sersic fits.But we use the $M_I$ values for 23.5 sample and 25.2 sample to see where the incompleteness kicks in.})
...
redshift evolution, \s$2$ includes  more galaxies. (\claire{How many?}\arun{Table. ~\ref{table:GalaxyCounts} gives the numbers bin-wise. Do you think we should mention the total number of relative increase here?})
...
function(\claire{the LF as a function of redshift, right?} \arun{I think we should say `determine the rate of evolution'?}) for our sample, thus simplifying this exercise.  However,
...
I don't see how the normalizations are different. Our selection criteria are different and hence the differences. })
...
because it was in the same field of view. Agreed, it doesn't play any crucial role. Errorbars doesn't help. They appear very small in log-log plot.}
          )
          }
 \label{fig:smf}
\end{figure*}
...
\claire{Maybe don't use red and green. It's not an uncommon color-blindness.} \arun{Use black instead of green maybe? Cyan and Magenta aren't very contrasting from blue and red resp.} 
...
environmentally neutral when we volume-limit the sample to be complete upto\claire{highlight} \arun{should we use `till'} $z=1$.
...
dominant in its size, the combination with (\claire{of} \arun{I guess this was meant to say COSMOS + other surveys?} )other much smaller fields
