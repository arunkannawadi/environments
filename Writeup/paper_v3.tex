...
Software packages like GalSim\footnote{\url{https://github.com/GalSim-developers/GalSim}} can generate galaxy images fairly accurately, given realistic input parametes.
...
is $0.12''$. High resolution images taken through the wide F814W filter (broad \emph{I}), after correcting for the PSF, have led to collection of postage stamp images\footnote{\url{http://irsa.ipac.caltech.edu/data/COSMOS/images/galaxy-postage-stamps/}} for the GREAT3 challenge~\citep{great3}, using the procedure described in \cite{SHERA}.
...
Thus the sample $z\le1$ and $M_\text{F814W}<-22$ is at least 97.84\% complete. Figs.~\ref{fig:2Dhist} \&~\ref{fig:MIhist} show that at $M_\text{F814W}<-22$, we are not affected by flux limit yet. 
...
\begin{table}
 ...
 \begin{tabular}[\columnwidth]{ | r | c | c | c | c | }
  ...
  Redshift bins & Tests & \s$1$ & \s$2$ & \s$3$ \\
  \hline
  All overdense vs. & K-S & 0.00056 & 0.0001 & 0.0 \\
  All underdense    & A-D & 3e-5 & 1e-5 & 0.0 \\ \hline 
  $z:[0.65,0.75]$ (OD) vs. & K-S & 0.9563 & 0.7476 & 0.5359 \\
  $z:[0.80,0.85]$ (OD) & A-D & 0.5162 & 0.3352 & 0.2290 \\ \hline
  $z:[0.65,0.75]$ (OD) vs & K-S & 0.00605 & 0.00025 & 0.00024 \\
  $z:[0.55,0.65]$ (UD) & A-D & 0.01201 & 0.00025 & 5e-5 \\ \hline
 \end{tabular}
 \caption{$p$-values from the Kolmogorov-Smirnov (K-S) and Anderson-Darling (A-D) obtained by comparing the ellipticities computed using second moments are given for three cases: \emph{all} overdense (OD) vs. \emph{all} underdense (UD), two overdense bins, not very separated in redshift and a pair of adjacent overdense and underdense bins. \s$1$, \s$2$, \s$3$ refer to the volume-limited sample in three different ways.  \rachel{Change numbers that are below 0.01 to scientific notation.}}
