...
\rachel{How does this figure look if you show the plot range with the
  ...
  remind the reader that you only fit above that redshift.}
}
...
For our (flux-limited) sample of galaxies, up to $z=1.0~(1.25)$
(\rachel{fix this to the correct one once we make final decisions}), we fit
...
\begin{equation}\label{E:pz1}
 p_1(z) \propto z^{a-1}\exp\left[{-bz}\right]
\end{equation}
and 
...
Our first approach is to generate a volume-limited sample that is
...
population (\rachel{What do you mean by a single stellar population?})
...
analytical functions with best fit parameters are plotted over it. 
           ...
           flux-limited sample redshift distribution plot.}}
...
\subsubsection{Comparing distributions}\label{subsubsec:compdist}
\begin{figure*}
 \centering
 \includegraphics[width=2.2\columnwidth]{axis_ratio_all}
...
\begin{figure*}
 ...
 \caption{Galaxy axis ratio
   distributions in two overdense redshift bins, $z=0.65-0.75$ and
   $z=0.80-0.85$, to check for consistency in the case that the
...
\begin{figure*}
 ...
 \caption{Galaxy axis ratio distributions in a single underdense redshift
   slice, $z=0.55-0.65$, and a single overdense redshift slice,
   $z=0.65-0.75$. The $p$-values from the
...
\begin{table}
 ...
 \begin{tabular}[\columnwidth]{ | l | c | c | c | }
  ...
  $[0.80,0.85]$ (OD) & 0.494 & 0.237 & 0.130 \\ \hline
 ...
 \caption{$p$-values from the Kolmogorov-Smirnov (top) and
   ...
   different types of volume-limited samples.
          The Anderson-Darling $p$-values are correct only up to 5
          decimal places. 
...
\begin{table}
 ...
 \begin{tabular}[\columnwidth]{ | l | l | l | l | }
  ...
  $[0.80,0.85]$ (OD) & 0.5162 & 0.3352 & 0.2290 \\ \hline
 ...
 \caption{$p$-values from the Kolmogorov-Smirnov (top) and
   ...
   Table~\ref{table:pvalues_all}.  \arun{Should I write only two
     ...
     comment applies to the previous table, too.}
           The Anderson-Darling $p$-values are correct only up to 5 decimal places. }
...
% \includegraphics[width=1.0\columnwidth]{rms_ellip2_noevolution.eps} \\
 \caption{RMS distortions as a function of redshift. The horizontal errorbars indicate
          ...
          environmental classification, as shown in the legend.  \rachel{The title and figure clearly show two
          samples.  The caption needs to explain which are solid
          points and which are open. Also, the horizontal axis label should
          be either $z$ or ``Photometric redshift ($z$)'', not ``Photo
          $z$'' (which is not a phrase we have used throughout the
          paper).  The change in horizontal axis label is needed for
          the other RMS ellipticity plots and anywhere else that it
...
%\includegraphics[width=1.0\columnwidth]{rms_ellip2_Bbandevolution.eps} \\
 \caption{RMS distortions as a function of redshift. The solid
   ...
   $-1.23$ magnitudes per unit redshift (\s$2$) and the open
          ...
          cuts (\s$3$). \rachel{If I compare this with
...
\rachel{Overall, I find it quite confusing that the underdense regions
  ...
  more naturally match up point color with galaxy type?}
...
For the luminosity-selected sample without any evolution (\s1), the
RMS distortions (Eq.~\ref{eq:distortion}) of galaxies in each redshift bin are shown
in Fig.~\ref{fig:rms_ellip}.  (\rachel{The figure seems to show two
  samples, not just \s1.  Update the text to reflect this?}) For this and all similar figures, the
colors of the points were selected to easily differentiate between
galaxies in overdense, neutral, and underdense redshift slices.
As shown, the underdense regions
have  higher values for RMS ellipticities when compared to the
...
evolution across the $z>0.5$ range.  Our conclusions are very similar
if we use the RMS ellipticity from Eq.~\eqref{eq:ellipticity} instead
of the distortion. (\rachel{Our statements at the low redshift end
...
higher RMS ellipticity. \rachel{Important note: this argument was
  ...
  this in future since it's a key point behind this work.}
...
When the $B$-band luminosity evolution is taken into account in
selecting the sample, a systematic increase in the ellipticity at
lower redshifts can be observed. We plot these along with stellar-mass
selected samples, where a similar trend is found, in
Fig.~\ref{fig:rms_ellip_Bband_mass}.

Finally, we show an analogous plot of RMS ellipticities for all three
...
For the other morphological parameters that we described in
...
$0.05<B/T<0.95$).  The KS and AD statistical tests are both sensitive
to exactly what happens at these hard boundaries in the distributions.
...
luminosity evolution when volume-limited the samples (\s1 and
\s2). Both panels also show the results with stellar mass-selected
samples (\s3) for reference.  
When using the stellar mass-selected sample, we observe that the overdense regions tend to have higher \sersic
...
surprising that it has a higher median \sersic $n$.
...
\begin{figure}
 ...
 \caption{Median values of the \sersic indices for volume-limited
   samples \s$1$ and \s$3$ are plotted (filled centers and thin
   errorbars) in top and bottom panels, respectively, for each
   redshift bin. 
          Median values for the \s$2$ sample are plotted in both the
...
\begin{figure}
 ...
 \includegraphics[width=1.0\columnwidth]{median_dvcbtt2}
  \caption{\rachel{Same confusion here as in previous figure caption;
      please clarify!} Median values of the bulge-to-total ratios for volume-limited samples \s$1$ and \s$3$ are plotted (filled centers and thin errorbars) in left and right panels respectively for each redshift bin.
...
\rachel{Here is one thing that is still missing from the results
  ...
  trends in the shape distributions disappear.  You had shown me plots
  and numbers for these, but they don't seem to have gone into the
  paper.  Please include them, because it would be useful to be able
  to give some guidance on how wide the redshift slices should be to
  avoid this issue.}
...
\rachel{Note to self: mention UDF, and tie back to bias in shear
  estimates?  Also, tie back to flux-limited samples, and why we
  should care about these results for them.}
