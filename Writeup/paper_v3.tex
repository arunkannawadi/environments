...
The typical variation in RMS ellipticity \arun{distortion?}\rachel{(no, too jargony for an abstract)} due to environmental effects
is 5 per cent \rachelnew{(absolute, not relative)} for redshift bins of width $\Delta z=0.05$, which could result in uncertain shear calibration at the 1 per cent level. We conclude that the cosmic variance effects are large enough to exceed the systematic error budget of future surveys, but can be mitigated with careful choice of training dataset and sufficiently large redshift binning.
\end{abstract}
...
matter power spectrum \rachelnew{\citep[e.g.,][]{2011MNRAS.415.3649V,2011MNRAS.417.2020S}}, as well as observational uncertainties such as
...
Some types of information about and tests of shear estimation
   ...
   \arun{I found this a little confusing. Rephrase it as ``Some types of information about the shear estimation and its tests for ongoing ... '', maybe?}   
\rachel{I find ``its tests'' confusing and very atypical usage.  The tests don't really belong to it; they are our tests OF shear estimation, hence the original phrasing.} 
 ...
 \rachelnew{\citep[see, e.g.,][]{2014MNRAS.439.1909V}.}
...
vs.\ ellipticals \arun{ellipticals refer to `bulge' here?} \rachel{yes, when you're talking about different galaxy morphologies, the standard terminology is disk vs.\ elliptical, whereas when you're talking about separate galaxy components, it's more common to say ``bulge''}), sizes, $S/N$, and shape \citep{2010MNRAS.405.2044B,2012MNRAS.423.3163K}. 
...
the undesired consequence of including \arun{a} variation in galaxy properties
...
and discuss their implications \rachelnew{in Sec.~\ref{S:implications},
...
\rachelnew{and we defer classication of the somewhat ambiguous range from
...
As \arun{done} for $M_I$, we compare the stellar mass function of the $m_\text{F814W}\le23.5$ sample with that of the $m_\text{F814W}\le25.2$ sample. 
...
section~2.1 of \cite{HS03} and \cite{BJ02}. \arun{($M_I$ is being reused.)} \rachel{You could slightly change the notation for moments, to $M_{(I)}$ and $M_{(g)}$, so that the symbol is different.} We refer to the
...
gradients \arun{color gradients?}\rachel{No, not at all, I mean ellipticity gradients.  If you have a flattened disk and a roundish bulge then the ellipticity you measure will change with the effective radius of the measurement.  See, for example, Bernstein (2010) for more info.}  are important, we could get different results using these
...
simulations, or whether our use of an area \arun{as large as} the size of COSMOS will
...
galaxies which are close to \arun{being} (\rachel{no, not grammatically correct; either ``close to round'' or ``close to round in shape'', but definitely not ``close to being round''}) round (large axis ratio and low RMS ellipticity/distortion). 
...
\rachelnew{Finally, we comment on whether the trends we
...
environmentally neutral when we \rachelnew{volume-limit the sample 
...
\rachelnew{As a test of the second option, we consider whether we can use some division of the
galaxies by color or $B/T$ (as a proxy for color) in order to
...
$B/T\ge $ or \arun{$<0.5$} .  The resulting bulge fraction $f_\text{bulge}$,
...
} \arun{$f_\text{bulge}$ was already used in a different context!} \rachel{Not any more.}
\begin{figure}
 \centering
 \includegraphics[width=1.0\columnwidth]{fbulge_z} 
 \caption{\rachelnew{The fraction of galaxies in \s1 (\arun{solid} symbols) and
     \s3 (\arun{open} symbols) that are classified as bulge-dominated as a
...
\rachelnew{Fig.~\ref{fig:rmsellip_bulge_disk} shows the RMS distortion as
...
\begin{figure}
 ...
 \caption{\rachelnew{The RMS distortion as a function of redshift for the
...
\rachel{As mentioned previously, {\em HST} data will be used by
\rachelnew{  current and} future weak
...
\rachel{The estimates in this subsection are rough illustrations of
  ...
  \rachelnew{The presence of other fields besides AEGIS (e.g., GOODS)
    ...
    by a small number of them\footnote{\rachelnew{For context,
        \cite{2015APh....63...81N} 
...
\rachelnew{However, it is important to bear in mind that the method
...
complicated analysis that is beyond the scope of this work.} It seems that a future study using
  ...
  test whether \rachelnew{or not our results suggest a discrepancy with those
    of 
...
\subsection{\rachel{Effective impact on \rachelnew{current and} future surveys}}
...
\rachel{In addition to the order of magnitude of these effects
  ...
  mind how \rachelnew{current and} future surveys plan to use {\em HST} data.}
...
\rachel{However, once the Euclid survey is
  ...
  of Euclid). \rachelnew{ The findings of \cite{2014MNRAS.439.1909V} demonstrate that the area of
...
\rachel{In contrast, for ground-based surveys such
  ...
  \rachelnew{Once the Euclid deep field is publicly available, the information
  from it will be beneficial to ground-based surveys as well.}

\rachelnew{Finally, our results for the effect on the intrinsic
...
\rachelnew{factor of 9 (for AEGIS, and more for GOODS).  Combinations
...
important to take into account.  \rachelnew{It would also be advisable to
...
\rachel{The authors would like to thank Thomas Kitching, Henk
  Hoekstra, and Tim Schrabback for their helpful comments on this work.}
