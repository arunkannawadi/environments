...
\newcommand{\rachel}[1]{{\textcolor{blue}{#1}}}
...
In Figs.~\ref{fig:rms_ellip}--\ref{fig:median_dvcbtt}, this \claire{is not always the case. (sometimes it is)} does not appear to be the case. 
...
To eliminate the \claire{possibility}plausibility (with great confidence) that this varying sample size is 
%not 
responsible for the trend observed, we repeated our analysis with only 50 per cent of overdense galaxies that were selected randomly. 
