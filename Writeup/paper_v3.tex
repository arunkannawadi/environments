...
\begin{figure}
 \centering
  ...
  \caption{Upper panel: Redshift distribution of flux-limited
    ...
    that are 0.05 wide. Two analytical functions, $p_1(z)$ and $p_2(z)$, defined in Eqns.~\ref{E:pz1} \&~\ref{E:pz2} with best fit
...
\rachel{Change $x$ axis label to ``Photometric redshift ($z$)''.  Use  thicker line width for the actual histogram.  Change legend to say  ``$p_1(z)$ (Eq. 1)'' and ``$p_2(z)$ (Eq. 2)'' rather than giving the actual equation.  Also, why are two of the horizontal lines in the bottom panel solid and one dashed?  What decides the style for eachline?  Finally, I think it makes sense to plot $\delta$ rather than $1+\delta$.}}
\arun{Done. But I refer to the equations in the caption instead of the legend.}
...
For our (flux-limited) sample of galaxies, up to $z=1.0(1.25)$, we fit
...
\begin{figure}
  \centering
   ...
   \caption{2D histogram of galaxies in apparent magnitude
     ...
     The green vertical dotted line represents our luminosity cut. \rachel{What redshift range?}\arun{addressed.}} 
   \label{fig:2Dhist}
\end{figure}
...
\begin{figure}
  \centering
   ...
   \rachel{Is this plot and the numbers in the caption for
     $0.8<z<0.85$ only?  This is currently implied by the text.  If
     that is true, then please fix the figure and caption to say
     this.  If that is wrong, then please fix the text.} \arun{Text is correct. Caption fixed.}}
   \label{fig:MIhist}
 \end{figure}
...
definitions of volume-limited samples. (\rachel{Do the numbers in the
  table include the full sample or just the subsample with postage stamps?} \arun{The subsample alone. The only place where the full sample is used is Fig. 3. The following sentence is corrected to say that.}) However, everywhere else in the paper, we consider only those galaxies for which there are postage stamp
...
The PSF correction scheme used on the observed images is that of `re-Gaussianization' method described in Sec.~2.4 of \cite{HS03} (hereafter HS03).
...
covariances of the measured object and PSF respectively, described in Sec.~2.1 of HS03, which is in turn based on \cite{BJ02}. Once we obtain the covariance matrices of the instrinsic image $f$, one can compute the ellipticities 
of the galaxies, which we will refer to as `ellipticities based on moments'.

\section{Results}
\label{S:results}
Having identified the overdense and underdense regions in a volume-limited sample (Sec.~\ref{sub:overdensities}, Sec.~\ref{sub:volumelimiting}), we will now see in this section if the morphological
parameters of the galaxies, listed in Sec.~\ref{sub:axisratio}, have any dependence on the environment that they reside in. We have 3 different ways of volume-limiting our sample
\begin{enumerate}
