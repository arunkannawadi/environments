...
\arun{Done.}
Major surveys such as the Hyper Suprime-Cam \arun{How to add shortforms?}~\citep{HSC_Overview}, Dark Energy Survey~\citep{DESC}, the KIlo-Degree Survey~\citep{KIDS}, the Panoramic Survey Telescope and Rapid Response System~\citep{PanSTARRS_2010},
the Large Synoptic Survey Telescope~\citep{LSST_Book},Euclid\footnote{http://sci.esa.int/science-e/www/area/index.cfm?fareaid=102}, Wide-Field Infrared Survey Telescope~\citep{WFIRST_Final}
...
\end{tabular}
\caption{List of different redshift bins, their environmental classification and the number of galaxies in them for 3 different ways of constructing a volume-limited sample. See text for more details.}
...
\begin{table}
 ...
 \end{tabular}
 \caption{\arun{I can't get the vertical lines in MNRAS:( \\}
 $p$-values from the Kolmogorov-Smirnov (K-S) and Anderson-Darling (A-D) obtained by comparing the distributions of axis ratios are given for three cases: \emph{all} overdense (OD) vs. \emph{all} underdense (UD), two overdense bins, not very separated in redshift and a pair of adjacent overdense and underdense bins. \s$1$, \s$2$, \s$3$ refer to the three ways of generating a volume-limited sample (See \S\ref{sub:volumelimiting}).
          }
...
\begin{figure*}
 \centering
 \includegraphics[width=2.3\columnwidth]{axis_ratio_all_all}
 ...
 \caption{The distributions of axis ratios in the case of luminosity-selected sample (left), luminosity-selected with B-band evolution taken into account (center) and stellar-mass-selected sample (right). The upper panels show the histogram
