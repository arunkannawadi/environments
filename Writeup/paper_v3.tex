...
cosmic variance could cause the galaxy population in them
...
estimation algorithms using sophisticated image simulations that
attempt to mimick real data by including
realistic galaxy populations based on high-resolution data from the
Hubble Space Telescope ({\em HST}).  However, existing datasets
from the {\em HST} cover very small cosmological volumes and cosmic variance could cause the galaxy populations in them
...
statistical properties of shape (axis ratio or ellipticity) and
morphological parameters (\sersicn and bulge-to-total ratio)
are influenced by redshift-dependent cosmic variance. We must distinguish between environment effects from large-scale structure, which we do not wish to include in simulations, and general trends in the galaxy population with redshift. The typical variation in RMS ellipticity due to environmental effects is XXX for redshift bins of width 0.05. We conclude that the cosmic variance effects are large enough to exceed the systematic error budget of future surveys, but can be mitigated with careful choice of training dataset and sufficiently large redshift binning.}
\rachel{Say something quantitative here once we find space.}
\end{abstract}
...
the ability to robustly infer shears from galaxy observed\claire{strike} shapes or
photometric redshifts from their observed\claire{strike} colors. (\arun{I don't see any problems with `observed'. Maybe just say observed galaxy shapes?}\claire{nothing wrong per se, I just don't think it's necessary, and was looking to shorten the sentence. It should definitely be 'observed galaxy shapes', not 'galaxy observed shapes'.})
...
representing\claire{strike; COSMOS \emph{is} the widest survey, it doesn't just represent it} the current widest contiguous area surveyed by the {\em HST}, and the
...
(\claire{I'm not sure how the parent sample comes into play here} 
 \arun{The catalog is upto 25.2. We restrict to 23.5 since fainter ones have bad Sersic fits.But we use the $M_I$ values for 23.5 sample and 25.2 sample to see where the incompleteness kicks in. }\claire{I understand this much better without the old Figure 2, so I think it's fine now.})
...
redshift evolution, \s$2$ includes  more galaxies. (\claire{How many?}\arun{Table. ~\ref{table:GalaxyCounts} gives the numbers bin-wise. Do you think we should mention the total number of relative increase here? }\claire{Yes, I do, even just in parentheses at the end of the sentence.})
...
function(\claire{the LF as a function of redshift, right?} \arun{I think we should say `determine the rate of evolution'?}\claire{your suggestion sounds fine}) for our sample, thus simplifying this exercise.  However,
...
$5\sigma$ depth of 24.9. (\claire{I think that's only true at the low mass end. Overall, your normalizations are different, which leads to the curve offsets. I'm not sure I would show the Tomczak curve since you don't really use it, and explaining the differences isn't so important.
...
\begin{figure*}
 ...
 \caption{Stellar mass distribution for various flux-limited samples
   ...
   bins have been chosen to facilitate the comparison with  
          ...
          (\claire{Why does your mass function go above Tomczak at higher redshift? If you aren't comparing to it directly, maybe just leave it out and do an internal comparison. Would including errors bars make the agreement better?}
           \arun{I guess Rachel wanted the Tomczak plot 
because it was in the same field of view. Agreed, it doesn't play any crucial role. Errorbars doesn't help. They appear very small in log-log plot.}\claire{see my extra notes in the text about this figure.}
          )
          }
 \label{fig:smf}
\end{figure*}
...
tricky\claire{highlight, I'd use a different word only because I think tricky is too informal. Unless the journal complains, its a personal taste thing. } \rachel{(agree, let's go with ``difficult'' or something like that)} because both distributions have hard cutoffs that are enforced
...
environmentally neutral when we volume-limit the sample to be complete upto\claire{highlight} \arun{should we use `till'}\claire{'up to' is two words, otherwise it's fine} $z=1$.
...
dominant in its size, the combination with (\claire{of} \arun{I guess this was meant to say COSMOS + other surveys?} \claire{how about this: However, as COSMOS is significantly larger than other \emph{HST} surveys, combining COSMOS with smaller fields is unlikely to ameliorate this effect.} \rachel{Sounds good to me.} )other much smaller fields
...
\claire{Kavli IPMU is supported by World Premier International Research Center Initiative (WPI), MEXT, Japan.}
