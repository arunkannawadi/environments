...
the Large Synoptic Survey Telescope~\citep[LSST;][]{LSST_Book}, Euclid\footnote{\url{http://sci.esa.int/euclid/}, \url{http://www.euclid-ec.org}} (\rachel{cite http://adsabs.harvard.edu/abs/2011arXiv1110.3193L}), Wide-Field Infrared Survey Telescope~\citep[WFIRST;][]{WFIRST_Final}
...
\rachel{Need to rephrase, at least a little.  I recommend being
  briefer and saying to see that paper for more details.}
\end{enumerate}
...
redshift histograms. \rachel{(To avoid ambiguity, you should use avoid
  ...
  the need for a normalization for which you are not actually fitting.)} Here $a,b$ and $c$ are free parameters that are to be determined.
...
(\rachel{Call it $\delta_g$, not $\delta$, which implies matter
...
\rachel{Say how this connects to the 3 samples S1, S2, and S3 that you
  defined earlier.}
