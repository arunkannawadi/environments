...
\begin{figure}
 \centering
  ...
  \caption{Upper panel: Redshift distribution of flux-limited
    ...
    that are 0.05 wide. The vertical line at $z=0.3$ indicates the delineation between lower redshifts that we do not use for our analysis, and higher redshifts that are used. Fits to two analytical functions, $p_1(z)$ and $p_2(z)$, defined in Eqns.~\ref{E:pz1} and~\ref{E:pz2}, are also shown, with best-fitting parameters $a=2.53\pm0.98$, $z_1=0.32\pm0.16$, $b=2.70\pm0.50$ and $z_2=0.63\pm0.13$.  We also show the distributions from \protect\cite{2004ApJ...617..765C} from Eqs.~\ref{E;pz3} and~\ref{E;pz4}. 
...
The functional forms for the (flux-limited) redshift distribution that we used in Sec.~\ref{sub:overdensities} are not physically motivated. We try to fit them again to the  
redshift distribution of a volume-limited sample ($M_I < -22$). 
Due to the absence of an exponential tail in the histogram, the parameters that set the scale for the redshift, $z_1$ and $z_2$ run to 
incredible large values during the fit, making $p_1(z)$ and $p_2(z)$ defined in Eq.~\eqref{E:pz1} and Eq.~\eqref{E:pz2} essentially the same power law.
The best fit exponent is remarkably close to $2$ suggesting that the number of galaxies within a given comoving volume grows proportional to the volume itself. 
...
The best fit parameters for $p_1(z)$ are $a=2.78\pm0.28$.
...
Next, we show an analogous plot of RMS ellipticities %\arun{Is this ellipticity or distortion?} 
for all three
