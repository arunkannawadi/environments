...
vs. ellipticals), sizes, $S/N$, and shape.  
 Realistic simulations use samples based on images from the Hubble
 Space Telescope ({\em HST}) such as COSMOS \citep[used by the GREAT3 challenge,][]{great3} or the
 Ultra Deep Field \citep[UDF, used by][]{2013ApJ...765...74J}.  These
 two examples serve as the extremes in the {\em HST} samples used as
 the basis for image simulation, with the former being shallower but
 the current widest contiguous area surveyed by the {\em HST}, and the
 latter being extremely deep but narrow.
The goal of this work is to understand the systematic uncertainties
that might arise due to the finite size of these areas, resulting in
cosmic variance that could result in selection of atypical galaxy
samples, particularly when dividing the sample as a function of
redshift. Upcoming surveys will study lensing as a function of
...
With this in mind, the GREAT3 challenge~\citep{great3} was held with the aim to test methods for measuring weak gravitational lensing by analyzing astronomical images.
...
viewed by some lower resolution telescope.
The training dataset used in the challenge comes from the COSMOS
survey, which we will describe in \S\ref{S:data}.
...
observed in the COSMOS field may (given the small size of the field)
...
due to the COSMOS galaxy sample in the simulated galaxies, rather than
only including ensemble effects such as true redshift evolution of
